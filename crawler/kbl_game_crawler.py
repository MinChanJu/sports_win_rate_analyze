...
async def kbl_game_crawler(URL, file_path, full_log=False):
    ...
    if results:
        ...
        if not full_log:
            print(
                f"KBL {'/'.join(URL.split('/')[-2:])} ({results[-1]['home']['name']} vs {results[-1]['away']['name']}) 크롤링 완료, quarters :[{', '.join(results[-1]['quarters'])}] => '{file_path}'에 저장됨"
            )
    else:
