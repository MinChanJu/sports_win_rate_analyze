...
async def crawl_kbl_match_chart(URL):
    # 1. URL 파싱
    try:
        parts = URL.split("/")
        gameKey = parts[-2]
        ...
        try:
            ...
            try:
                ...
                async with page.expect_response(
                    lambda response: TEAM_INFO_KEYWORD in response.url
                ...
                team_response = (
                    await team_resp_info.value
                )  # 1. 응답 객체(Response)를 먼저 받음
                ...
                if "arr_game" in team_json and len(team_json["arr_game"]) > 0:
                    ...
                    metainfo["home"]["code"] = str(
                        game_info.get("HOME_TEAM", "")
                    ).strip()
                    metainfo["away"]["code"] = str(
...
def save_results_to_file(metainfo, chart_data, file_path):
    ...
    json_data = {"metainfo": metainfo, "shootLog": chart_data if chart_data else []}
...
async def main():
    test_URL = "https://kbl.or.kr/match/record/S47G01N86/20251205"

    metainfo, chart_data = await crawl_kbl_match_chart(test_URL)

    print("Metainfo:")
    print(json.dumps(metainfo, ensure_ascii=False, indent=2))
    print("\nChart Data Sample:")
    if chart_data:
        print(
            json.dumps(chart_data[:3], ensure_ascii=False, indent=2)
        )  # 상위 3개 항목만 출력
    else:
