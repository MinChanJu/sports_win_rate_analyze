...
from kbl_game_shoot_crawler import kbl_chart_crawler
import asyncio, os, shutil


async def handle_response(res: Response, folder: str):
    try:
        data = await res.json()
        for match in data:
            if match["seasonCategoryName"] != "정규시즌":
                continue
            ...
            if gmkey and gameDate:
                ...
                await kbl_chart_crawler(url, file_path)
    except:
        print("Error occurred while processing response")


async def main(s: int, e: int, folder: str, reset: bool = False):
...
if __name__ == "__main__":
    ...
    asyncio.run(main(7, 51, "../kbl_chart_data", True))
