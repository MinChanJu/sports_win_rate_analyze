            ...
            for file in files:
                ...
                if match:
                    ...
                    with open(file_path, "r") as f:
                        ...
                        check_log += f", last quarter: {json_content['logs'][-1]['q'] if 'logs' in json_content and len(json_content['logs']) > 0 else 'N/A'}"
                        if (
                            "logs" not in json_content
                            or len(json_content["logs"]) == 0
                            or not (
                                json_content["logs"][-1]["q"] == "Q4"
                                or json_content["logs"][-1]["q"].startswith("X")
                            )
                        ):
                            check_log += " ❌\n"
                            wrong_files.append(
