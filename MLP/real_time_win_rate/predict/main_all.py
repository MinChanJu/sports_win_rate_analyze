from data import load_state, load_multi_csv_all
...
def main():
    ...
    if (args.predict_split):
        df, drop_df  = load_multi_csv_all(predict_csv_list, model_info.get("data_split", {"test_game_keys": []}).get("test_game_keys", []))
    else:
        df, drop_df = load_multi_csv_all(predict_csv_list, None)
    ...
    report_path = Path(args.model_dir) / Path(args.model_type) / f'predict_report.json'
    with open(report_path, "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"[saved] metadata -> {report_path}")
    
    confusion_matrix_path = Path(args.model_dir) / Path(args.model_type) / f'predict_confusion_matrix.png'
    save_confusion_matrix(report_path, confusion_matrix_path)
